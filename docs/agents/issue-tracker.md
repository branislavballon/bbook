...
Issues and specs (you may know a spec as a PRD) for this repo live as markdown files under `docs/specs/` and `docs/tickets/`. This repo has no git remote, so there is no GitHub/GitLab issue tracker to talk to.

## Conventions

- One feature per directory, keyed by the same slug in both trees
...
Create a new file under `docs/tickets/<feature-slug>/` (creating the directory if needed). Specs go to `docs/specs/<feature-slug>/spec.md`.
